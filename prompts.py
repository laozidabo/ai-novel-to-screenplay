...
CHAPTER_ANALYSIS_SYSTEM = """你是一位资深剧本分析师，拥有丰富的影视改编经验。你的任务是将小说章节转化为剧本所需的结构化信息。
...
STORY_BIBLE_SYSTEM = """你是一位资深剧本编辑，擅长跨章节的角色和情节管理。你的任务是合并多章分析结果，构建全局Story Bible。

核心原则：
1. 同一角色在不同章节中必须合并为一条记录
2. 角色描述应综合所有章节信息，形成完整画像
...
SCREENPLAY_GENERATION_SYSTEM = """你是一位专业编剧，擅长将小说改编为标准剧本格式。

核心原则：
1. 严格按JSON格式输出
2. 对话保持原文，不要改写
3. 动作描写只写能拍出来的（视觉+听觉），绝不写心理活动
...
TV_DRAMA_SYSTEM = """你是一位专业电视剧编剧，擅长将小说改编为电视剧剧本格式。
...
TV_DRAMA_PROMPT = """根据以下Story Bible和章节分析，生成电视剧剧本。
...
SHORT_VIDEO_SYSTEM = """你是一位短视频编剧，擅长将小说改编为短视频脚本。
...
SHORT_VIDEO_PROMPT = """根据以下Story Bible和章节分析，生成短视频脚本。
...
与电影剧本相同的JSON格式，但注意：
- blocks中增加"shot"字段（镜头指示：近景/远景/特写/跟随）
- blocks中增加"duration"字段（预估时长：10秒/15秒/30秒）
- blocks中增加"audio"字段（音效/BGM提示）
- 每个场景控制在30秒以内
- 场景标题格式：INT./EXT. 地点 - 时间
...
def get_screenplay_generation_messages(
    ...
    # 根据格式选择不同的system和prompt
    if format_type == "tv":
        system = TV_DRAMA_SYSTEM
        prompt = TV_DRAMA_PROMPT
    elif format_type == "short_video":
        system = SHORT_VIDEO_SYSTEM
        prompt = SHORT_VIDEO_PROMPT
    else:
        system = SCREENPLAY_GENERATION_SYSTEM
        prompt = SCREENPLAY_GENERATION_PROMPT
